...
public interface ISubscriptionGateway
{
    ...
    Task<SubscriptionWebhookSecret?> GetWebhookSecretAsync(string topicName, Guid subscriptionId);
    Task<SubscriptionWebhookSecret?> RotateWebhookSecretAsync(string topicName, Guid subscriptionId);
}